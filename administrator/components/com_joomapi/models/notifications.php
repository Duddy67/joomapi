...
class JoomapiModelNotifications extends JModelList
{
  /**
   * Constructor.
   *
   ...
   *
   * Notification. Calling getState in this method will result in recursion.
   *
  ...
  protected function getListQuery()
  {
    ...
    $query->select($this->getState('list.select', 'n.id,n.title,n.alias,n.created,n.published,n.catid,n.hits,'.
	  ...
	  ->from('#__joomapi_notification AS n');
    ...
    if($hasTag) {
      $query->join('LEFT', $db->quoteName('#__contentitem_tag_map', 'tagmap').
		   ...
		   ' AND '.$db->quoteName('tagmap.type_alias').' = '.$db->quote('com_joomapi.notification'));
    }
